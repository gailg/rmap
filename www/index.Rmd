...
with subsequent observed outcomes. `rmap` offers a grouped goodness-of-fit test and grouped and individualized attribute diagrams. Concordance (also called the area under the ROC curve) measures how well a model separates or discriminates positive and negative outcomes.`rmap` offers estimation and confidence intervals for the concordance as well as ROC plots.  As of version 0.03.00, `rmap` handles three designs for obtaining samples of participants: (1) random sampling, (2) two-stage sampling, and (3) weighted designs.
...
In the example above and in examples below, I use `N_bootstraps = 100` so you can run through these examples relatively quickly.  When you perform your own analyses, I suggest starting with `N_bootstraps = 100` for organizing your analyses, and then changing to higher numbers to obtain your final results.  
...
All RStudio code chunks except the two inside the section [An example that compares two risk models](#comparing-two-models) are "self-contained.  This means that you can try out any code chunk by stepping through any one, just start at the beginning of the code chunk and send each line to R. You will not need any definitions from a previous code chunk.  The only exception is the last pair of code chunks (inside [An example that compares two risk models](#comparing-two-models)), where you will need to start at the beginning of the first code chunk.  (I broke the code into two chunks because I wanted the figures from each chunk to be sized differently.)
...
If your data were obtained by two-stage sampling, `category` is the two-stage category indicating which two-stage category the person fell into. (See also `N_first_stage` below.)  If your data are a cohort sample whose relevant covariates do not match those of your target population, and you would like to weight your cohort sample based on covariate information from a target population, `category` records which covariate category the (cohort) individual fell into. If your data were obtained by a random sample, you will not need `category`. 
...
If your data were obtained using a two-stage sample design, your will need `N_first_stage`.  If your data were obtained using a weighted design, you will need either `N_target` or `target_category`.  Otherwise, you have a random sample and you can skip this section.
...
you wish risk groups is to be defined using `K` quantiles, the assigned risks of each risk group is to be summarized using their mean.
...
Use this input to specify your study design and any necessary details to perform analyses based on your study design.  
...
If you would like `rmap` to break down your sample into `K` groups, for example `K = 4` (so the risks are broken down into quartiles) you can specify `risk_group = list(K = 4)`. (Throughout this web page, and even when you are not using qunatiles to define your risk groups, I denote `K` to be the number of risk groups.)
...
This nonnegative integer specifies the number of bootstraps.  If `N_bootstraps = 0`, no bootstraps will be performed. For random samples and two-stage samples, grouped goodness-of-fit tests and concordance estimates can be gotten without allowing bootstraps, but for inferences on concordance as well as inferences on all quantities when the design is a weighted sample, you will need bootstrapping.
...
and `pi_hat` which estimates the probability, given being inside each risk group, of getting the outcome before `t_star` and before the competing risk.
...
If you have a random sample, and you are happy using assigned risk quantiles to define your risk groups, the mean to summarize the risks in each risk group, and confidence level `0.95`,  you can use the wrapper functions `rmap_random_sample` and `individual_random_sample` as illustrated in the Introduction.
...
# A repeat of the previous example, but with different variable names
...
I see that `my_design` is a list that contains two objects, named `category` and `target_category`. These objects get their names and how they are populated from their definition in the list 
`my_design = list(category = my_category, target_category = my_target_category)`.
...
When you hand `my_design` to `rmap`, the objects named `target` and `target_category` signal to `rmap` that you want to do a weighted analysis and it should understand that `category` contains the covariate categories of the cohort sample and `target_category` contains the covariate categories of the target sample.
...
I see that `my_risk_group` is a list that contains the object `cutoffs`, which gets its name and what is inside it from its definition in the list `my_risk_group = list(cutoffs = my_cutoffs)`.
...
```{r compare, fig.height = 3, fig.width = 4}
library(rmap)
...
[doc-concordance-formulas.pdf](doc-concordance-formulas.pdf)
