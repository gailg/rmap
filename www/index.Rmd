...
with subsequent observed outcomes. `rmap` offers a grouped goodness-of-fit test and grouped and individualized attribute diagrams. Concordance (also called the area under the ROC curve) measures how well a model separates or discriminates positive and negative outcomes.`rmap` offers estimation and confidence intervals for the concordance as well as ROC plots.  Version 0.03.00 of `rmap` handles three designs for obtaining samples of participants: (1) random sampling, (2) two-stage sampling, and (3) weighted designs.

Below I show some R code for reading in some simulated data, calls to `rmap` functions, and a display showing `rmap` in action:  estimating concordance, producing ROC plots, testing for grouped goodness-of-fit, and producing grouped and individualized attribute diagrams.
...
In the example above and in all examples below, I use `N_bootstraps = 100` so you can run through these examples relatively quickly.  When you perform your own analyses, I suggest using `N_bootstraps = 100` for organizing your analyses, and then changing to higher numbers to obtain your final results.  

The html page that you are looking at was created in RStudio.  You may download the RStudio file [rmap-tutorial.Rmd](rmap-tutorial.Rmd), open it in RStudio and follow along. 
All RStudio code chunks except the two inside the section [An example that compares two risk models](#comparing-two-models) are "self-contained (you do not need any definitions from previous code chunks to try out the code in any code chunk except in [An example that compares two risk models](#comparing-two-models), the second code chunk depends on the one immediately before it).
...
One of the required steps below is to install `devtools`.  Before that, if your operating system is Windows, you will need to install Rtools, and if your operating system is Mac OS, you will need to install Xcode and command-line tools:
...
If your data were obtained using a two-stage sample design, your will need `N_first_stage`.  If your data were obtained using a weighted design, you will need to either `N_target` or `target_category`.  Otherwise, you have a random sample and you can skip this section.
...
you wish risk groups is to be defined using `K` quantiles, (where `K` is a positive integer), the assigned risks of each risk group is to be summarized using their mean.
...
This positive number is the right end point of the time period of interest.  The personal risk model that you are asking `rmap` to evaluate expresses the risk, or the probability of the outcome within the time period `[0, t_star]` and before a prespecified competing risk.
...
The examples used in this webpage use `N_bootstraps = 100` so that the examples can be stepped through relatively quickly.  You should use much larger numbers, for example `N_bootstraps = 400` or `N_bootstraps = 1000`.
