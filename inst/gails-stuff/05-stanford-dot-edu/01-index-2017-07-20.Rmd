...
For you to see `rmap in action, I show here a very simple call to `rmap` on a simulated random sample.


```{r message = FALSE, echo = FALSE, fig.height = 6, fig.widht = 7}
library(rmap)
...
r = xxx$r
t_star = 10
rmap_2 = rmap_ungrouped_too(e, t, r, t_star)
the_message = paste("Estimate concordance and produce ROC plots,",
                    "(below),",
                    "test for goodness of fit,",
                    "and produce grouped and ungrouped", 
                    "attribute diagrams", 
                    "(right)",sep = "\n")
...
If your data is an excel file with columns `e`, `t`, and `r`, save it as a CSV (comma separated value) file into your working directory. For practice you can download my excel file here, put it any your working directory, and try out the following commands.
...
If you got the same results I above, you are in business.  
...
r = xxx$r
t_star = 10
rmap_1 = rmap_simplest_example(e, t, r, t_star)
grid.arrange(rmap_1$roc_plot, rmap_1$grouped_risk_plot, 
             ncol = 2, 
             top = "A very simple example of rmap in action")
```

# And if you wish to add an ungrouped summary

```{r eval = FALSE, fig.height = 5, fig.width = 7}
set.seed(1)
rmap_2 = rmap_ungrouped_too(e, t, r, t_star)
the_message = paste("The function rmap_ungrouped_too,",
                    "which produces the same results",
                    "as the simple example above",
                    "but ungrouped has been added", sep = "\n")
grid.arrange(textGrob(the_message),  rmap_2$grouped_risk_plot, 
             rmap_2$roc_plot, rmap_2$ungrouped_risk_plot,
             ncol = 2)
```
...
The functions `rmap_simplest_example` and `rmap_ungrouped_too` are wrapper functions that call `rmap` the main function of the `rmap` package.  For example the following lines either appear in `rmap_ungrouped_too` or are defined by default, and they show an example of the kind of input required by `rmap`. In the following sections, I describe `rmap` inputs and outputs and then proceed to some examples.
...
Once you have defined appropriately `e`, `t`, `t_star`, `design`, `risk_group`, `r_summary`, `N_bootstraps`, `confidence_level`, `N_cores`,  and `verbose`, then use this call to `rmap`
...
grouped `rmap` output is broken down into two objects: `numerical_summaries` and `plots`.  `numerical_summaries` consist of the following
