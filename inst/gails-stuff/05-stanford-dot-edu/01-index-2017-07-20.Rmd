...
The functions `rmap_simplest_example` and `rmap_ungrouped_too` are wrapper functions that call `rmap` the main function of the `rmap` package.  For example the following lines either appear in `rmap_ungrouped_too` or are defined by default, and they show an example of the kind of input required by `rmap`.
