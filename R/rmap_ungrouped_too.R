rmap_ungrouped_too = function(e, t, r, t_star, K = 4, N_bootstraps = 100){
  ...
  #------------------------------------------------------------------- ungrouped risk
  ungrouped_risk_plot = if(N_bootstraps > 0){
    epsilon = length(e)^(-1/3)
    risk_group = list(epsilon = epsilon)
    ungrouped = rmap(e, t, r, t_star, design, risk_group, r_summary, 
                     N_bootstraps, confidence_level, N_cores, verbose)
    ungrouped$plots$risk_plot +
      labs(title = "Ungrouped Attribute Diagram",
           subtitle = " ")
  } else {
    NULL
  }
  list(roc_plot = roc_plot,
       ...
       ungrouped_risk_plot = ungrouped_risk_plot)
}